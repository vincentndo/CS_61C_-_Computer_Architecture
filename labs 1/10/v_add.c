...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     ...
     #pragma omp parallel
	{
		...
		for(int id=omp_get_thread_num(); id<ARRAY_SIZE/num*num; id+=num)
			z[id] = x[id] + y[id];
	}
	
	for (int i = ARRAY_SIZE/num*num; i<ARRAY_SIZE; i++)
	{
		z[i] = x[i] + y[i];
	}
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
	...
	{
		num = omp_get_num_threads();
		chunk = ARRAY_SIZE/num;
		for(int id=omp_get_thread_num()*chunk; id<omp_get_thread_num()*chunk + chunk; id++)
			z[id] = x[id] + y[id];
	}
