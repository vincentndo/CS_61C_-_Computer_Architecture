...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
    if (strcmp(name, "li") == 0) {
        ...
        int err = translate_num(&imm, args[1], LONG_MIN, ULONG_MAX)
        if (err == -1)
        {
        	return -1
        }
        ...
        {
        	err = translate_num(&imm, args[1], 0, USHRT_MAX)
        	if (err == -1)
        	{
        		int upper = imm & (~(1<<16));
        		int lower = imm & (1<<16);
        		fprintf(output, "lui %s %d\n", args[0], upper );
        		fprintf(output, "ori %s %d\n", args[0], lower);
        		return 2
        	}
        	else
        	{
        		fprintf(output, "addiu %s %d\n", args[0], imm );
        		return 1;
        	}
        }
        ...
        fprintf(output, "addu $t0 %s %s\n", args[1], args[2]);
    	fprintf(output, "addu %s %s $t0\n", args[3], args[3]);
        return 2;      
        ...
        fprintf(output, "addu $t0 %s $0\n", args[0]);
    	fprintf(output, "addu %s %s $0\n", args[0], args[1]);
    	fprintf(output, "addu %s $t0 $0\n", args[1]);
        return 3;
...
int translate_inst(FILE* output, const char* name, char** args, size_t num_args, uint32_t addr,
    ...
    else if (strcmp(name, "beq") == 0)   return write_branch (0x04, output, args, num_args, addr, reltbl);
    else if (strcmp(name, "bne") == 0)   return write_branch (0x05, output, args, num_args, addr, reltbl);
 ...
 //   else if (strcmp(name, "jal") == 0)   return write_shift (0x00, output, args, num_args);
...
int write_rtype(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = funct + rd<<11 + rt<<16+ rs<<21 + 0<<26;
...
int write_shift(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = funct + shamt<<6 + rd<<11 + rt<<16 + 0<<26;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_jr(uint8_t funct, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = funct + rs<<21 + 0<<26;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_ori(uint8_t opcode, FILE* output, char** args, size_t num_args) {
    ...
    uint32_t instruction = rt<<16 + rs<<21 + opcode<<26;
    write_inst_hex(output, instruction);
    return 0;
}
...
int write_jump(uint8_t opcode, FILE* output, char** args, size_t num_args, uint32_t addr, SymbolTable* reltbl) {
    /* YOUR CODE HERE */
    int label_addr = get_addr_for_symbol(symbol, args[0]);
    
    int32_t offset = (label_addr - addr +4)/4;
    uint32_t instruction = offset + opcode<<26;
    write_inst_hex(output, instruction);
    return 0;
}
