...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
        ...
        int is_label = add_if_label(input_line, str, byte_offset, symtbl);
        if (is_label==0)
        {
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    // so you don't need to worry about that here.
        ...
        while (num_args<3)
        {
            ...
            char* curr_arg = args[num_args];
            if (curr_arg[sizeof(curr_arg)-1]="\n")
            {
                break;
            }
            num_args = num_args +1;
    ...
    // raise_inst_error(). 
        err = translate_inst(output, name, args, num_args, lnum*4, symtbl, reltbl);
        if (err = -1)
        {
