...
void simple_status_test(void)
{
  // This is to test beargit status
  // The test checks the return value of the beargit status call and its output to stdout.
  // In case of correct implementation, the expected retval = 0 and output message as described in the specs.
...
int cunittester()
{
   ...
   if (NULL == CU_add_test(pSuite4, "Status output test", simple_status_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
