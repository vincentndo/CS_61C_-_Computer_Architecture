...
void test_write_functions() {
    ...
    char* args[] = {"$t0", "$t1", "-56"};
    write_addiu (0x09, output, args, 3);

    // addu
    // char* args1[] = {"$t0", "$s0", "$s1"}; 
    // size_t s = 3;   
    // write_rtype(0x21, output, args1, s);

    // // sll
    // char* args2[] = {"$t1", "$s1", "5"};    
    // s = 3;
    // write_shift(0x00, output, args2, s); 

    // // jr
    // char* args3[] = {"$s0"};
    // s = 1;
    // write_jr(0x08, output, args3, s);

    // write_shift(0x00, output, args2, 3); 
    ...
    // printf("%s\n", line);
    // CU_ASSERT(!strcmp(line, "02114021"));

    // // sll
    // fgets(line, 9, output);
    // strtok(line, "\n");
    // CU_ASSERT_PTR_NOT_NULL(line);
    // CU_ASSERT(!strcmp(line, "00114940"));

    // // jr
    // fgets(line, 9, output);
    // strtok(line, "\n");
    // CU_ASSERT_PTR_NOT_NULL(line);
    // CU_ASSERT(!strcmp(line, "03a00008"));
...
int main(int argc, char** argv) {
    ...
    pSuite3 = CU_add_suite("Testing write functions in translate.c", NULL, NULL);
    if (!pSuite3) {
        goto exit;
    }
    if (!CU_add_test(pSuite3, "test_write_functions", test_write_functions)) {
        goto exit;
    }
